...
/**
 * 클라이언트가 고를 수 있는 친구 목록 정렬. 요청의 {@code sort=name,asc} 같은 문자열을 허용 목록으로만 해석한다.
...
public enum FriendSort {
    ...
    public List<OrderSpecifier<?>> orders() {
        return switch (this) {
            case NAME_ASC -> List.of(nameGroup(0, 1).asc(), nameKey().asc(), member.email.asc(), member.id.asc());
            case NAME_DESC -> List.of(nameGroup(1, 0).asc(), nameKey().desc(), member.email.desc(), member.id.desc());
            case EMAIL_ASC -> List.of(member.email.asc(), member.id.asc());
            case EMAIL_DESC -> List.of(member.email.desc(), member.id.desc());
        };
    }

    /**
     * 이름을 세 그룹으로 나눈다. 이름 없음(null·빈 문자열)은 방향과 상관없이 항상 마지막(2).
    ...
    /** 소문자로 비교해 대소문자 구분이 다른 H2/MySQL 에서 같은 순서가 나오게 하고, null 은 빈 문자열과 같게 본다. */
    private static ComparableExpressionBase<String> nameKey() {
        return member.username.lower().coalesce("");
    }
}
