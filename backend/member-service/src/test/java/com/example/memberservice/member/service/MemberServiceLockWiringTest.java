...
class MemberServiceLockWiringTest {
    ...
    @Test
    void findOrCreate_isAnnotatedWithApiLock_andItsParameterWithLockParam() throws NoSuchMethodException {
        Method findOrCreate = MemberService.class.getMethod("findOrCreateGoogleMember", GoogleAccountDto.class);

        assertThat(findOrCreate.getAnnotation(ApiLock.class)).isNotNull();

        Parameter[] parameters = findOrCreate.getParameters();
        assertThat(parameters).hasSize(1);
        assertThat(parameters[0].getAnnotation(LockParam.class)).isNotNull();
    }

    @Test
    void findOrCreate_hasNoMethodLevelTransactional() throws NoSuchMethodException {
        Method findOrCreate = MemberService.class.getMethod("findOrCreateGoogleMember", GoogleAccountDto.class);
        ...
        assertThat(findOrCreate.getAnnotation(Transactional.class)).isNull();
    }
    ...
    void registerMember_noLongerCarriesApiLock() throws NoSuchMethodException {
        ...
        Method registerMember = MemberService.class.getDeclaredMethod("registerMember", GoogleAccountDto.class);
