...
public class MemberPublicController {
    ...
    /**
     * 친구 목록. filter(기본 normal)는 숨김·차단한 친구를 빼고, sort(기본 name,asc) 는 {@link FriendSort} 허용 목록만 받는다.
    ...
    public ResponseEntity<PageResponse<ResponseFriendDto>> friendsList(@Valid @PathVariable("userId") String userId,
        ...
        FriendFilter friendFilter = FriendFilter.parse(filter)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "지원하지 않는 필터입니다: " + filter));
        ...
        return ResponseEntity.ok(PageResponse.from(memberFriendService.getFriendsPage(userId, friendFilter, friendSort, pageable), f -> f));
    }

    /** 내가 차단한 친구들의 userId. 앱이 메시지·알림을 거를 때 쓴다. */
    @GetMapping("/{userId}/friends/blocked-ids")
    public ResponseEntity<List<String>> blockedFriendIds(@Valid @PathVariable("userId") String userId) {
        return ResponseEntity.ok(memberFriendService.getBlockedUserIds(userId));
    }
    ...
    public ResponseEntity<ResponseFriendDto> friend(@Valid @PathVariable("userId") String userId,
                                                    @PathVariable("friendMemberId") Long friendMemberId) {
        return ResponseEntity.ok(friendOrNotFound(friendMemberId, () -> memberFriendService.getFriend(userId, friendMemberId)));
    }

    /** 즐겨찾기 켜기/끄기. 차단한 친구면 400, 친구가 아니면 404. */
    @PutMapping("/{userId}/friends/{friendMemberId}/favorite")
    public ResponseEntity<ResponseFriendDto> setFavorite(@Valid @PathVariable("userId") String userId,
                                                         @PathVariable("friendMemberId") Long friendMemberId,
                                                         @RequestBody FriendFlagDto request) {
        return ResponseEntity.ok(friendOrNotFound(friendMemberId, () -> memberFriendService.setFavorite(userId, friendMemberId, request.isOn())));
    }

    /** 친구 숨기기/숨김 해제. 해제하면 NORMAL 로 돌아온다. */
    @PutMapping("/{userId}/friends/{friendMemberId}/hidden")
    public ResponseEntity<ResponseFriendDto> setHidden(@Valid @PathVariable("userId") String userId,
                                                       @PathVariable("friendMemberId") Long friendMemberId,
                                                       @RequestBody FriendFlagDto request) {
        return ResponseEntity.ok(friendOrNotFound(friendMemberId, () -> memberFriendService.setHidden(userId, friendMemberId, request.isOn())));
    }

    /** 친구 차단/차단 해제. 차단하면 즐겨찾기도 꺼진다. */
    @PutMapping("/{userId}/friends/{friendMemberId}/blocked")
    public ResponseEntity<ResponseFriendDto> setBlocked(@Valid @PathVariable("userId") String userId,
                                                        @PathVariable("friendMemberId") Long friendMemberId,
                                                        @RequestBody FriendFlagDto request) {
        return ResponseEntity.ok(friendOrNotFound(friendMemberId, () -> memberFriendService.setBlocked(userId, friendMemberId, request.isOn())));
    }

    /** 친구 행이 없으면(내 친구가 아니면) 404. 전역 예외 처리기가 없어서 여기서 바꾼다. */
    private ResponseFriendDto friendOrNotFound(Long friendMemberId, Supplier<ResponseFriendDto> action) {
        try {
            return action.get();
