package com.example.authservice.oauth.grant.google

import com.example.authservice.oauth.config.AuthorizationServerConfig
import com.example.authservice.oauth.grant.GrantRequestParser
import com.example.authservice.oauth.grant.GrantSupport
import jakarta.servlet.http.HttpServletRequest
import org.springframework.security.core.Authentication
import org.springframework.security.web.authentication.AuthenticationConverter

/** POST /oauth2/token grant_type=…google_id_token&client_id=…&id_token=… */
class GoogleIdTokenGrantConverter : AuthenticationConverter {

    override fun convert(request: HttpServletRequest): Authentication? {
        if (!GrantRequestParser.isGrant(request, AuthorizationServerConfig.GOOGLE_ID_TOKEN.value)) {
            return null
        }
        val client = GrantRequestParser.clientPrincipal()
        val idToken = GrantRequestParser.required(request, "id_token")
        return GoogleIdTokenGrantToken(
            client, idToken, GrantRequestParser.scopes(request),
            GrantSupport.additionalParameters(request.parameterMap),
        )
    }
}
