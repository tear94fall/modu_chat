 ...
 * 다루므로 커스텀 grant 와 refresh_token 은 여기서 받는다. 검증은 [PublicClientAuthenticationProvider].
 */
class PublicClientAuthenticationConverter : AuthenticationConverter {
    ...
    override fun convert(request: HttpServletRequest): Authentication? {
        ...
        val grant = request.getParameter(OAuth2ParameterNames.GRANT_TYPE)
        if (grant != null && !grant.startsWith("urn:modu:") && grant != "refresh_token") {
            return null
        }
        return OAuth2ClientAuthenticationToken(clientId, ClientAuthenticationMethod.NONE, null, null)
    }
}
